...
class Dataframe:
    ...
    def gerar_arquivo_excel(self, vouchers):
        print('\n\nGERANDO ARQUIVO EXCEL')
        print('-'*50)

        df = pd.DataFrame(columns=['nome','link','pontos','valor','milheiro'])

        for categoria in vouchers.keys():
            for produto in vouchers[categoria]:
                df_nova_linha = pd.DataFrame([produto])
                df = pd.concat([df, df_nova_linha])

        #salva o arquivo organizado em ordem decrescente do milheiro
        df = df.sort_values('milheiro', ascending=False)

        #cria o documento .xlsx
        writer = pd.ExcelWriter('azul_vouchers.xlsx', engine='xlsxwriter')

        #armazena os dados no arquivo e planilha exata
        df.to_excel(writer, sheet_name='Azul', index=False)
...
excel = Dataframe()
excel.gerar_arquivo_excel(vouchers)
